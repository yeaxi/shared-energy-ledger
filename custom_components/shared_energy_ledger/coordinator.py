...
class SharedEnergyLedgerCoordinator(DataUpdateCoordinator[CoordinatorPayload]):
    ...
    async def _async_update_data(self) -> CoordinatorPayload:
        ...
        if result.tenants is not None:
            ...
            # Commit accounting anchors only for a priced interval so an
            # unavailable tick never loses metered energy.
            committed_anchors = {**committed_anchors, **current_samples}

        ledger_current: dict[str, float] = {}
        ledger_reset_keys: set[str] = set()
        unpriceable_charge = False
        if config.battery is not None:
            ledger_charge = self._delta(
                ledger_anchors,
                ledger_current,
                LEDGER_ANCHOR_CHARGE,
                battery_charge,
                reset_keys=ledger_reset_keys,
            )
            ledger_discharge = self._delta(
                ledger_anchors,
                ledger_current,
                LEDGER_ANCHOR_DISCHARGE,
                battery_discharge,
            ...
            ledger_pv: float | None
            if config.pv is not None:
                ledger_pv = self._delta(
                    ledger_anchors,
                    ledger_current,
                    LEDGER_ANCHOR_PV,
                    pv_gen,
                ...
                ledger_pv = 0.0  # no-silent-zero: allow (PV not configured)
            mix_consumption = building_consumption_from_balance(
                ledger_grid, ledger_pv, ledger_discharge, ledger_charge
            )
            mix = price_charge_mix(
                ChargeMixInputs(
                    consumption_kwh=mix_consumption,
                    charge_kwh=ledger_charge,
                    pv_configured=config.pv is not None,
                    pv_generation_kwh=ledger_pv if config.pv is not None else 0.0,  # no-silent-zero: allow (PV not configured)
                    pv_price=pv_price,
                    grid_price=grid_price,
                )
            )
            unpriceable_charge = (
                ledger_charge is not None
                and ledger_charge > 1e-9
                and mix.charge_unit_cost is None
            )
            new_ledger = await self._advance_ledger(
                ...
                ledger_charge,
                ledger_discharge,
                mix.charge_unit_cost,
            )
            if new_ledger is not None:
                payload.ledger = new_ledger

        commit_ledger_anchors = bool(ledger_current) and (
            not unpriceable_charge or bool(ledger_reset_keys)
        )
        if commit_ledger_anchors and not unpriceable_charge:
            ledger_anchors.update(ledger_current)
