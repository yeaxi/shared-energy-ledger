...
ISSUE_CONFIG_INVALID: Final = "config_invalid"
...
def raise_config_invalid(hass: HomeAssistant, entry_id: str, reason: str) -> None:
    """Raise a repair when the stored configuration fails validation."""
    ir.async_create_issue(
        hass,
        DOMAIN,
        _issue_id(entry_id, ISSUE_CONFIG_INVALID),
        is_fixable=False,
        severity=ir.IssueSeverity.ERROR,
        translation_key=ISSUE_CONFIG_INVALID,
        translation_placeholders={"reason": reason},
    )


def clear_config_invalid(hass: HomeAssistant, entry_id: str) -> None:
    ir.async_delete_issue(hass, DOMAIN, _issue_id(entry_id, ISSUE_CONFIG_INVALID))
